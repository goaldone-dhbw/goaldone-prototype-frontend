...
import { MembersService, InvitationsService, MemberResponse, InvitationResponse } from '../../api';
...
export class SettingsComponent implements OnInit {
  ...
  loadMembers(page: number = this.membersPage()) {
    ...
    this.membersService.listMembers(page, this.membersSize).pipe(
  ...
  removeMember(userId: string) {
    this.membersService.removeMember(userId).pipe(
      catchError(error => {
        if (error.status === 404) {
          this.messageService.add({
            key: 'settings-toast',
  ...
  loadInvitations(page: number = this.invitationsPage()) {
    ...
    this.invitationsService.listInvitations(page, this.invitationsSize).pipe(
  ...
  sendInvitation() {
    ...
    this.invitationsService.createInvitation({ email: this.inviteEmail() }).pipe(
  ...
  revokeInvitation(invitationId: string) {
    this.invitationsService.revokeInvitation(invitationId).pipe(
      catchError(error => {
        if (error.status === 404) {
          this.messageService.add({
            key: 'settings-toast',
